...
По умолчанию он слушает только `127.0.0.1:8787`. Relay использует SQLite/WAL,
криптографическое подтверждение владения mailbox, TTL, глобальные и mailbox-
квоты и rate limiting. Не публикуйте его напрямую: перед широкой публичной
альфой ещё нужны HTTPS-прокси, проверенное резервное копирование, мониторинг и
раздельные отзываемые write-capabilities контактов. Шаблоны VPS находятся в
